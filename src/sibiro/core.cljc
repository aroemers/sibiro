...
(defrecord Match [handler path-params middleware uri])

(defprotocol RouteMatcher
  (find-match [routes ring-request]
    "Given a routes datastructure and a Ring request, return a Match
    record if it can be matched.")
  (all-matches [routes]
    "Given a routes datastructure, return a collection of all Match
    records."))
...
(defprotocol KeyHandler
  (update-ring-request [key ring-request]
    ...
    the type of the key. Returns nil if no match can be made.")
  (update-match [key match]
    "Given a map key and a Match record, updates it according to the
    type of the key."))
...
(extend-type clojure.lang.PersistentVector
  ...
  (update-match [[param-key param-coerce-fn] {:keys [uri path-params] :as match}]
    (assoc match
...
(extend-type clojure.lang.PersistentArrayMap
  RouteMatcher
  (find-match [routes {:keys [uri] :as ring-request}]
    (first (keep (fn [[matcher sub-routes]]
                   (when-let [updated-request (update-ring-request matcher ring-request)]
                     (some-> (find-match sub-routes updated-request)
                             (assoc :uri uri))))
                 routes)))
  (all-matches [routes]
    (reduce-kv (fn [a k v]
                 (let [handler-reqs (all-matches v)
                       updated      (map #(update-match k %) handler-reqs)]
                   (concat a updated)))
               nil routes)))
...
(extend-type clojure.lang.Fn
  ...
  (find-match [routes {:keys [uri path-params middleware]}]
    (when (or (= uri "") (= uri "/"))
      (Match. routes path-params middleware uri)))
  (all-matches [routes]
    [(Match. routes nil nil "")])

  KeyHandler
  (update-ring-request [key ring-request]
    (update ring-request :middleware conj key))
  (update-match [key match]
    (update match :middleware conj key)))
...
(extend-type java.lang.String
  ...
  (update-match [key match]
    (update match :uri #(str "/" key %))))


(defn mk-handler
  "Create a request handler for the given routes structure."
  [routes]
  (fn [request]
...
(defn mk-finder
  "Create a request maker for the given routes structure. Available
  handlers are only calculated once."
  [routes]
  (let [handlers (reduce #(assoc %1 (:handler %2) %2) {} (all-matches routes))]
    (fn finder
